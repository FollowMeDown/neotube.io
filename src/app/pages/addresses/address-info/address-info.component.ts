...
export class AddressInfoComponent implements OnInit, OnDestroy {
    ...
    handleChartData(data) {
        ...
        if (data === null || data === undefined || data.length === 0) {
            targetData.push({ x: this.formatDate(startTime), y: 0 });
            targetData.push({ x: this.formatDate(currentTime), y: 0 });
        } else {
            for (let i = 0, j = 0; i < this.chartOption; i++) {
                const leftTime = startTime + i * 24 * 3600;
                if (j === data.length) {
                    targetData[i] = { x: this.formatDate(leftTime), y: targetData[i - 1].y };
                    continue;
                }
