    ...
    handleChartData(data) {
        ...
        if (data === null || data === undefined || data.length === 0) {
            ...
            for (let i = 0, j = 0; i < this.chartOption; i++) {
                ...
                if (targetData.max) {
                    targetData.max = y > targetData.max ? y : targetData.max;
                } else {
                    targetData.max = y;
                }
        ...
        targetData.min = Math.floor(targetData.min);
        targetData.max = Math.floor(targetData.max);
        const distance = Math.ceil((targetData.max - targetData.min) / 8);
        let stepSize;
        if (distance <= 1) {
            stepSize = 1;
        } else if (distance <= 5) {
            ...
            const prefix = String(distance).slice(0, 2);
            let tempStep;
            if (Number(prefix[1]) >= 5) {
                tempStep = String(Number(prefix[0]) + 1) + '0';
            } else {
                tempStep = prefix[0] + '5';
            }
            let len = String(distance).length - 2;
            while (len > 0) {
                tempStep += '0';
                len--;
            }
            stepSize = Number(tempStep);
        }
        targetData.min = Math.floor(targetData.min / stepSize) * stepSize;
        return targetData;
    }
